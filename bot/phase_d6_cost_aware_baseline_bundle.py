...
D6_COST_AWARE_BASELINE_BUNDLE_PHASE = "D6_cost_aware_baseline_bundle_v1"
DEFAULT_COST_AWARE_BUNDLE_SCENARIOS = ["standard_fee_only"]
...
def normalize_candle_paths(paths: Iterable[str | Path]) -> List[str]:
    ...
    for raw in paths:
        ...
        out.append(text)
    if not out:
        raise ValueError("d6_cost_aware_bundle_requires_at_least_one_candle_file")
    return out


def normalize_cost_scenarios(values: Optional[Iterable[Any]] = None) -> List[str]:
    allowed = set(list_cost_scenarios())
    raw_values = list(DEFAULT_COST_AWARE_BUNDLE_SCENARIOS if values is None else values)
    out: List[str] = []
    seen: set[str] = set()
    for raw in raw_values:
        ...
        out.append(scenario)
    if not out:
        raise ValueError("d6_cost_aware_bundle_requires_at_least_one_cost_scenario")
    return out


def _compact_report(report: Dict[str, Any]) -> Dict[str, Any]:
    summary = dict(report.get("summary") or {})
    derived = dict(report.get("derived_costs") or {})
    ...
    return {
        ...
        "round_trip_cost_pct": derived.get("round_trip_cost_pct"),
        "entry_cost_pct": derived.get("entry_cost_pct"),
        "exit_cost_pct": derived.get("exit_cost_pct"),
...
def build_phase_d6_cost_aware_baseline_bundle(
    *,
    ...
    warnings = [
        "research_cost_aware_baseline_bundle_only",
    ...
    return {
        ...
        "phase": D6_COST_AWARE_BASELINE_BUNDLE_PHASE,
        "status": "d6_cost_aware_baseline_bundle_ready",
        "candle_paths": paths,
...
def bundle_to_markdown(bundle: Dict[str, Any]) -> str:
    ...
    lines = [
        "# D.6 Cost-Aware Baseline Bundle",
        "",
        ...
        f"- split_mode: `{bundle.get('split_mode', '')}`",
        f"- contains_rankings: `{summary.get('contains_rankings')}`",
        f"- contains_recommendations: `{summary.get('contains_recommendations')}`",
        "",
        "| product | timeframe | scenario | split_count | validation_return_pct | test_return_pct | worst_test_drawdown_pct | usable | blockers |",
        "| --- | --- | --- | ---: | ---: | ---: | ---: | --- | ---: |",
    ]
    for row in bundle.get("reports") or []:
        lines.append(
            "| {product_id} | {timeframe} | {cost_scenario} | {split_count} | {average_validation_net_return_pct} | {average_test_net_return_pct} | {worst_test_max_drawdown_pct} | {usable_for_future_research} | {blocker_count} |".format(
                **row
            )
        )
    lines.extend(
...
__all__ = [
    "D6_COST_AWARE_BASELINE_BUNDLE_PHASE",
    "DEFAULT_COST_AWARE_BUNDLE_SCENARIOS",
