...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build read-only replication lifecycle schema v1 report. No live replication and no Coinbase calls."
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    report = build_replication_lifecycle_schema_report(root=Path(args.root))
    if args.json_out:
        _atomic_write(
            Path(args.json_out),
    ...
    if args.markdown_out:
        _atomic_write(
            ...
            (render_replication_lifecycle_schema_markdown(report) + "\n").encode("utf-8"),
        )
    if args.markdown:
        print(render_replication_lifecycle_schema_markdown(report))
    else:
