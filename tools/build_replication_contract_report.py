...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build read-only replication/follower contract report. No live replication and no Coinbase calls."
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    report = build_replication_contract_report(root=Path(args.root))
    if args.json_out:
        _atomic_write(
            Path(args.json_out),
    ...
    if args.markdown_out:
        _atomic_write(Path(args.markdown_out), (render_replication_contract_markdown(report) + "\n").encode("utf-8"))
    if args.markdown:
        print(render_replication_contract_markdown(report))
    else:
