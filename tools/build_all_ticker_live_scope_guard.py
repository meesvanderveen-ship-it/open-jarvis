...
from bot.phase_all_ticker_live_scope_guard import build_all_ticker_live_scope_guard, render_all_ticker_live_scope_guard_markdown  # noqa: E402
...
def main() -> int:
    ...
    report = build_all_ticker_live_scope_guard(root=args.root)
    js = json.dumps(report, indent=2, sort_keys=True, ensure_ascii=True) + "\n"
    md = render_all_ticker_live_scope_guard_markdown(report)
    if args.json_out:
    ...
    if args.markdown:
        ...
        print(f"all_ticker_live_scope_guard ready={flags.get('all_ticker_live_scope_guard_ready')} classification={report.get('classification')}")
    return 0
