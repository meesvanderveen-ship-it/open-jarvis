...
from bot.phase_controlled_learning_governance import (  # noqa: E402
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build report-only controlled learning governance design.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    report = build_controlled_learning_governance_report(root=Path(args.root))
    json_text = json.dumps(report, indent=2, sort_keys=True, ensure_ascii=True) + "\n"
    markdown_text = render_controlled_learning_governance_markdown(report)
    if args.json_out:
    ...
    if args.markdown:
        ...
        print(
            ...
            f"ready={flags.get('controlled_learning_governance_ready')} "
            f"current_max_allowed_learning_stage={flags.get('current_max_allowed_learning_stage')} "
