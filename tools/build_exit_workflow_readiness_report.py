...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build read-only D2/D3/D4/D5 exit workflow readiness report. No live exits or Coinbase calls."
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    report = build_exit_workflow_readiness_report(root=Path(args.root))
    if args.json_out:
        _atomic_write(
            Path(args.json_out),
    ...
    if args.markdown_out:
        _atomic_write(
            ...
            (render_exit_workflow_readiness_markdown(report) + "\n").encode("utf-8"),
        )
    if args.markdown:
        print(render_exit_workflow_readiness_markdown(report))
    else:
