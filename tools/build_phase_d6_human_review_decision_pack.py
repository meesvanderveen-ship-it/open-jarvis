...
from bot.phase_d6_human_review_decision_pack import (  # noqa: E402
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the D6 human-review decision pack. Report-only: no Coinbase calls, no market-data fetch, "
            "no HTTP calls, no optimization, no ranking, no parameter proposal, no parameter mutation and no "
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    report = build_d6_human_review_decision_pack_report(root=Path(args.root))
    json_text = json.dumps(report, indent=2, sort_keys=True, ensure_ascii=True) + "\n"
    markdown_text = render_d6_human_review_decision_pack_markdown(report)
    ...
    if args.markdown:
        ...
        print(
            "d6_human_review_decision_pack "
            f"classification={report.get('classification')} "
            f"ready={flags.get('d6_human_review_decision_pack_ready')} "
            f"parameter_values_proposed={flags.get('parameter_values_proposed')} "
