...
from bot.phase_follower_receiver_api_audit import (  # noqa: E402
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build report-only follower receiver/API audit from local files. No HTTP calls, no Coinbase calls, "
            "no follower live enablement and no trading-state writes."
        )
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="Optional follower receiver/API path to inspect. May be supplied more than once.",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    report = build_follower_receiver_api_audit_report(
        root=Path(args.root),
        follower_paths=args.follower_path,
    )
    json_text = json.dumps(report, indent=2, sort_keys=True, ensure_ascii=True) + "\n"
    markdown_text = render_follower_receiver_api_audit_markdown(report)
    ...
    if args.markdown:
        ...
        print(
            "follower_receiver_api_audit "
            f"classification={report.get('classification')} "
            f"follower_receiver_code_accessible={flags.get('follower_receiver_code_accessible')} "
            f"follower_ready_for_live={flags.get('follower_ready_for_live')} "
            f"follower_buy_ready={flags.get('follower_buy_ready')} "
            f"follower_sell_ready={flags.get('follower_sell_ready')} "
