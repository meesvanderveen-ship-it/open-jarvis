...
from bot.phase_multi_ticker_paper_lifecycle_replay import (  # noqa: E402
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build report-only/paper-only multi-ticker lifecycle replay. No Coinbase calls, no HTTP calls, "
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    report = build_multi_ticker_paper_lifecycle_replay_report(root=Path(args.root))
    json_text = json.dumps(report, indent=2, sort_keys=True, ensure_ascii=True) + "\n"
    markdown_text = render_multi_ticker_paper_lifecycle_replay_markdown(report)
    ...
    if args.markdown:
        ...
        print(
            "multi_ticker_paper_lifecycle_replay "
            f"classification={report.get('classification')} "
            f"ready={gate.get('multi_ticker_paper_lifecycle_replay_ready')} "
            f"tickers_replay_partial_count={gate.get('tickers_replay_partial_count')} "
