...
from bot.phase_d6_oos_degradation import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="D.6 research-only out-of-sample degradation diagnostics. No Coinbase calls."
    )
    ...
    parser.add_argument("--output", default="", help="Optional JSON degradation output path")
    parser.add_argument("--markdown-output", default="", help="Optional Markdown degradation output path")
...
def main() -> int:
    ...
    if args.output:
        write_json_degradation(report, args.output)
    if args.markdown_output:
        write_markdown_degradation(report, args.markdown_output)
