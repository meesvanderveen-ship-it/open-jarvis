...
from bot.phase_d6_backlearning_multisource_v9 import (  # noqa: E402
...
from bot.phase_d6_btc_1h_staged_controller_v4 import (  # noqa: E402
...
DEFAULT_RESUME = "reports/d6/btc-1h-staged-resume-status-v5-20260601.json"
...
DEFAULT_MASTER = "reports/d6/24h-readiness-master-packet-v26-v27-20260601.json"
...
def _preflight_v13(*, plan: Dict[str, Any], result: Dict[str, Any] | None, quality: Dict[str, Any], binance_1h: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "generated_at": now_iso(),
        "phase": "D6_v28_v29_btc_1h_controller",
        "report_name": "24h_live_test_preflight_runner_v13",
        "status": "24h_live_test_preflight_v13_blocked",
        ...
        "blockers": ["btc_usdc_1h_staged_gap_fill_incomplete", "primary_coinbase_dataset_quality_not_ready", "normal_backtests_deferred", "future_live_test_ack_missing"],
...
def _master_v28_v29(
    *,
    ...
    return {
        "generated_at": now_iso(),
        "phase": "D6_v28_v29_btc_1h_controller",
        "report_name": "24h_readiness_master_packet_v28_v29",
        "status": "24h_readiness_v28_v29_not_ready",
        "btc_usdc_only_24h_readiness": {
            "closer_than_v26_v27": bool(completed),
        ...
        "no_go_boundaries": ["no_live_submit", "no_cancel_replace_reprice", "no_state_write", "no_config_or_parameter_mutation", "no_coinbase_account_or_order_endpoint", "no_binance_account_or_order_endpoint", "no_external_candle_as_coinbase_candle", "no_auto_resume_after_failure"],
...
def _index_v28_v29(*, outputs: Iterable[str], master: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "generated_at": now_iso(),
        "phase": "D6_v28_v29_btc_1h_controller",
        "report_name": "research_readiness_index_v28_v29",
        "status": "research_readiness_index_v28_v29_ready",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="D.6 BTC-USDC 1H staged controller v28/v29.")
    ...
    parser.add_argument("--candidate-root", default="/tmp/d6_btc_1h_staged_controller_v28_v29")
    ...
    plan = build_btc_1h_staged_controller_plan_v4(resume_status_path=args.resume_status, candidate_root=args.candidate_root, max_chunks=args.max_chunks)
    result = None
    if args.fetch:
        from bot.coinbase_client import CoinbaseClient  # noqa: E402

        result = execute_btc_1h_staged_controller_v4(
            plan=plan,
    ...
    resume_v6 = build_resume_status_v6(plan=plan, result=result)
    binance_1h = build_binance_btc_1h_reference_report_v5(controller_result=result)
    cross_source = build_cross_source_btc_1h_gap_diagnostic_v4(controller_result=result)
    quality = build_quality_summary_v28_v29()
    pattern_map = build_open_source_backlearning_pattern_map_v7()
    backlearning = build_backlearning_multisource_scaffold_v9(
        quality_summary=quality,
        controller_result=result or {},
        binance_reference=binance_1h,
        source_paths=[BTC_1H_CANDLES, args.binance_4h_reference],
    )
    backtest = build_exploratory_backtest_decision_v28_v29(quality_summary=quality)
    preflight = _preflight_v13(plan=plan, result=result, quality=quality, binance_1h=binance_1h)
    master = _master_v28_v29(plan=plan, result=result, resume=resume_v6, quality=quality, binance_4h=binance_4h, binance_1h=binance_1h, backlearning=backlearning, backtest=backtest)
    specs = [
        ("btc_1h_staged_controller_plan_v4", plan, "btc-1h-staged-controller-plan-v4-20260601"),
        ("btc_1h_staged_resume_status_v6", resume_v6, "btc-1h-staged-resume-status-v6-20260601"),
        ("binance_btcusdc_1h_reference_v5", binance_1h, "binance-btcusdc-1h-reference-v5-20260601"),
        ("cross_source_btc_1h_gap_diagnostic_v4", cross_source, "cross-source-btc-1h-gap-diagnostic-v4-20260601"),
        ("post_btc_1h_staged_quality_summary_v28_v29", quality, "post-btc-1h-staged-quality-summary-v28-v29-20260601"),
        ("open_source_backlearning_pattern_map_v7", pattern_map, "open-source-backlearning-pattern-map-v7-20260601"),
        ("backlearning_multisource_scaffold_v9", backlearning, "backlearning-multisource-scaffold-v9-20260601"),
        ("exploratory_only_backtest_decision_v28_v29", backtest, "exploratory-only-backtest-decision-v28-v29-20260601"),
        ("24h_live_test_preflight_runner_v13", preflight, "24h-live-test-preflight-runner-v13-20260601"),
        ("24h_readiness_master_packet_v28_v29", master, "24h-readiness-master-packet-v28-v29-20260601"),
    ]
    if result is not None:
        specs.insert(1, ("btc_1h_staged_controller_result_v4", result, "btc-1h-staged-controller-result-v4-20260601"))
    outputs: List[str] = []
    for report_type, content, stem in specs:
        outputs.extend(_write_pair(report_type, content, stem, source_paths=source_paths))
    research_index = _index_v28_v29(outputs=outputs, master=master)
    outputs.extend(_write_pair("research_readiness_index_v28_v29", research_index, "research-readiness-index-v28-v29-20260601", source_paths=[*source_paths, *outputs]))
    print(json.dumps({"status": "v28_v29_reports_written", "fetch": args.fetch, "completed_chunks": (result or {}).get("completed_chunk_ids", []), "outputs": outputs}, indent=2, sort_keys=True))
    return 0 if not result or result.get("status") == "btc_1h_staged_controller_result_v4_ready" else 2
