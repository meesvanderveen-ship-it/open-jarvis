...
from bot.phase_safe_regression_harness import (  # noqa: E402
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run local safe regression harness. Report-only: no Coinbase calls, no HTTP replication, "
            "no live action and no trading-state writes."
        )
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Reserved for future bounded report regeneration; current harness only references existing reports.",
    )
    parser.add_argument(
        "--include-selected-tests",
        action="store_true",
        help="Run the fixed local selected-test allowlist. No arbitrary commands are accepted.",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    report = build_safe_regression_harness(root=Path(args.root), include_selected_tests=args.include_selected_tests)
    json_text = json.dumps(report, indent=2, sort_keys=True, ensure_ascii=True) + "\n"
    markdown_text = render_safe_regression_harness_markdown(report)
    ...
    if args.markdown:
        ...
        print(
            "safe_regression_harness "
            f"classification={report.get('classification')} "
            f"local_safe_regression_passed={report.get('local_safe_regression_passed')} "
            f"selected_tests_enabled={(report.get('selected_tests') or {}).get('selected_tests_enabled')} "
            f"selected_tests_classification={(report.get('selected_tests') or {}).get('selected_tests_classification')} "
