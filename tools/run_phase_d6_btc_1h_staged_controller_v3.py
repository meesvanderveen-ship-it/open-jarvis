...
from bot.phase_d6_backlearning_multisource_v8 import (  # noqa: E402
...
from bot.phase_d6_btc_1h_staged_controller_v3 import (  # noqa: E402
...
DEFAULT_RESUME = "reports/d6/btc-1h-staged-resume-status-v4-20260601.json"
...
DEFAULT_MASTER = "reports/d6/24h-readiness-master-packet-v24-v25-20260601.json"
...
def _preflight_v12(*, plan: Dict[str, Any], result: Dict[str, Any] | None, quality: Dict[str, Any], binance_1h: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "generated_at": now_iso(),
        "phase": "D6_v26_v27_btc_1h_controller",
        "report_name": "24h_live_test_preflight_runner_v12",
        "status": "24h_live_test_preflight_v12_blocked",
...
def _master_v26_v27(
    *,
    ...
    return {
        "generated_at": now_iso(),
        "phase": "D6_v26_v27_btc_1h_controller",
        "report_name": "24h_readiness_master_packet_v26_v27",
        "status": "24h_readiness_v26_v27_not_ready",
        "btc_usdc_only_24h_readiness": {
            "closer_than_v24_v25": bool(completed),
...
def _index_v26_v27(*, outputs: Iterable[str], master: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "generated_at": now_iso(),
        "phase": "D6_v26_v27_btc_1h_controller",
        "report_name": "research_readiness_index_v26_v27",
        "status": "research_readiness_index_v26_v27_ready",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="D.6 BTC-USDC 1H staged controller v26/v27.")
    ...
    parser.add_argument("--candidate-root", default="/tmp/d6_btc_1h_staged_controller_v26_v27")
    ...
    plan = build_btc_1h_staged_controller_plan_v3(resume_status_path=args.resume_status, candidate_root=args.candidate_root, max_chunks=args.max_chunks)
    result = None
    if args.fetch:
        from bot.coinbase_client import CoinbaseClient  # noqa: E402

        result = execute_btc_1h_staged_controller_v3(
            plan=plan,
    ...
    resume_v5 = build_resume_status_v5(plan=plan, result=result)
    binance_1h = build_binance_btc_1h_reference_report_v4(controller_result=result)
    cross_source = build_cross_source_btc_1h_gap_diagnostic_v3(controller_result=result)
    quality = build_quality_summary_v26_v27()
    pattern_map = build_open_source_backlearning_pattern_map_v6()
    backlearning = build_backlearning_multisource_scaffold_v8(
        quality_summary=quality,
        controller_result=result or {},
        binance_reference=binance_1h,
        source_paths=[BTC_1H_CANDLES, args.binance_4h_reference],
    )
    backtest = build_exploratory_backtest_decision_v26_v27(quality_summary=quality)
    preflight = _preflight_v12(plan=plan, result=result, quality=quality, binance_1h=binance_1h)
    master = _master_v26_v27(plan=plan, result=result, resume=resume_v5, quality=quality, binance_4h=binance_4h, binance_1h=binance_1h, backlearning=backlearning, backtest=backtest)
    specs = [
        ("btc_1h_staged_controller_plan_v3", plan, "btc-1h-staged-controller-plan-v3-20260601"),
        ("btc_1h_staged_resume_status_v5", resume_v5, "btc-1h-staged-resume-status-v5-20260601"),
        ("binance_btcusdc_1h_reference_v4", binance_1h, "binance-btcusdc-1h-reference-v4-20260601"),
        ("cross_source_btc_1h_gap_diagnostic_v3", cross_source, "cross-source-btc-1h-gap-diagnostic-v3-20260601"),
        ("post_btc_1h_staged_quality_summary_v26_v27", quality, "post-btc-1h-staged-quality-summary-v26-v27-20260601"),
        ("open_source_backlearning_pattern_map_v6", pattern_map, "open-source-backlearning-pattern-map-v6-20260601"),
        ("backlearning_multisource_scaffold_v8", backlearning, "backlearning-multisource-scaffold-v8-20260601"),
        ("exploratory_only_backtest_decision_v26_v27", backtest, "exploratory-only-backtest-decision-v26-v27-20260601"),
        ("24h_live_test_preflight_runner_v12", preflight, "24h-live-test-preflight-runner-v12-20260601"),
        ("24h_readiness_master_packet_v26_v27", master, "24h-readiness-master-packet-v26-v27-20260601"),
    ]
    if result is not None:
        specs.insert(1, ("btc_1h_staged_controller_result_v3", result, "btc-1h-staged-controller-result-v3-20260601"))
    outputs: List[str] = []
    for report_type, content, stem in specs:
        outputs.extend(_write_pair(report_type, content, stem, source_paths=source_paths))
    research_index = _index_v26_v27(outputs=outputs, master=master)
    outputs.extend(_write_pair("research_readiness_index_v26_v27", research_index, "research-readiness-index-v26-v27-20260601", source_paths=[*source_paths, *outputs]))
    print(json.dumps({"status": "v26_v27_reports_written", "fetch": args.fetch, "completed_chunks": (result or {}).get("completed_chunk_ids", []), "outputs": outputs}, indent=2, sort_keys=True))
    return 0 if not result or result.get("status") == "btc_1h_staged_controller_result_v3_ready" else 2
