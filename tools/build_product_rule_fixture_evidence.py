...
from bot.phase_product_rule_fixture_evidence import (  # noqa: E402
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build local/report-only product-rule fixture evidence. No Coinbase calls, no market-data fetch, "
            "no HTTP calls, no parameter mutation and no trading-state writes."
        )
    )
    parser.add_argument("--root", default=str(PROJECT_ROOT), help="Project root; defaults to this repository.")
    parser.add_argument("--fixture-path", default="", help="Optional local fixture JSON path.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    report = build_product_rule_fixture_evidence_report(
        root=Path(args.root),
        fixture_path=args.fixture_path or None,
    )
    json_text = json.dumps(report, indent=2, sort_keys=True, ensure_ascii=True) + "\n"
    markdown_text = render_product_rule_fixture_evidence_markdown(report)
    ...
    if args.markdown:
        ...
        print(
            "product_rule_fixture_evidence "
            f"classification={report.get('classification')} "
            f"fixture_completion_ready={summary.get('fixture_completion_ready')} "
            f"fixture_evidence_ticker_count={summary.get('fixture_evidence_ticker_count')} "
