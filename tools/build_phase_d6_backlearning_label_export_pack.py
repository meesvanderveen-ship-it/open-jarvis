...
from bot.phase_d6_backlearning_label_export_pack import (  # noqa: E402
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build report-only D6 backlearning label export pack.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    report = build_d6_backlearning_label_export_pack_report(root=Path(args.root))
    json_text = json.dumps(report, indent=2, sort_keys=True, ensure_ascii=True) + "\n"
    markdown_text = render_d6_backlearning_label_export_pack_markdown(report)
    if args.json_out:
    ...
    if args.markdown:
        ...
        print(
            ...
            f"ready={flags.get('d6_backlearning_label_export_pack_ready')} "
            f"export_record_count={counts.get('export_record_count')} "
