...
from bot.phase_d6_backlearning_multisource_v11 import (  # noqa: E402
...
from bot.phase_d6_btc_1h_staged_controller_v6 import (  # noqa: E402
...
DEFAULT_RESUME = "reports/d6/btc-1h-staged-resume-status-v7-20260601.json"
...
DEFAULT_MASTER = "reports/d6/24h-readiness-master-packet-v30-v31-20260601.json"
...
def _preflight_v15(*, plan: Dict[str, Any], result: Dict[str, Any] | None, quality: Dict[str, Any], binance_1h: Dict[str, Any]) -> Dict[str, Any]:
    completed = list((result or {}).get("completed_chunk_ids") or [])
    return {
        "generated_at": now_iso(),
        "phase": "D6_v32_v33_btc_1h_controller",
        "report_name": "24h_live_test_preflight_runner_v15",
        "status": "24h_live_test_preflight_v15_blocked",
...
def _master_v32_v33(
    *,
    ...
    return {
        "generated_at": now_iso(),
        "phase": "D6_v32_v33_btc_1h_controller",
        "report_name": "24h_readiness_master_packet_v32_v33",
        "status": "24h_readiness_v32_v33_not_ready",
        "readiness_answers": {
        ...
        "btc_usdc_only_24h_readiness": {
            "closer_than_v30_v31": bool(completed),
...
def _index_v32_v33(*, outputs: Iterable[str], master: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "generated_at": now_iso(),
        "phase": "D6_v32_v33_btc_1h_controller",
        "report_name": "research_readiness_index_v32_v33",
        "status": "research_readiness_index_v32_v33_ready",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="D.6 BTC-USDC 1H staged controller v32/v33.")
    ...
    parser.add_argument("--candidate-root", default="/tmp/d6_btc_1h_staged_controller_v32_v33")
    ...
    plan = build_btc_1h_staged_controller_plan_v6(resume_status_path=args.resume_status, candidate_root=args.candidate_root, max_chunks=args.max_chunks)
    result = None
    if args.fetch:
        from bot.coinbase_client import CoinbaseClient  # noqa: E402

        result = execute_btc_1h_staged_controller_v6(
            plan=plan,
    ...
    resume_v8 = build_resume_status_v8(plan=plan, result=result)
    binance_1h = build_binance_btc_1h_reference_report_v7(controller_result=result)
    cross_source = build_cross_source_btc_1h_gap_diagnostic_v6(controller_result=result)
    quality = build_quality_summary_v32_v33()
    pattern_map = build_open_source_backlearning_pattern_map_v9()
    backlearning = build_backlearning_multisource_scaffold_v11(
        quality_summary=quality,
        controller_result=result or {},
        binance_reference=binance_1h,
        source_paths=[BTC_1H_CANDLES, args.binance_4h_reference],
    )
    backtest = build_exploratory_backtest_decision_v32_v33(quality_summary=quality)
    preflight = _preflight_v15(plan=plan, result=result, quality=quality, binance_1h=binance_1h)
    master = _master_v32_v33(plan=plan, result=result, resume=resume_v8, quality=quality, binance_4h=binance_4h, binance_1h=binance_1h, backlearning=backlearning, backtest=backtest)
    specs = [
        ("btc_1h_staged_controller_plan_v6", plan, "btc-1h-staged-controller-plan-v6-20260601"),
        ("btc_1h_staged_resume_status_v8", resume_v8, "btc-1h-staged-resume-status-v8-20260601"),
        ("binance_btcusdc_1h_reference_v7", binance_1h, "binance-btcusdc-1h-reference-v7-20260601"),
        ("cross_source_btc_1h_gap_diagnostic_v6", cross_source, "cross-source-btc-1h-gap-diagnostic-v6-20260601"),
        ("post_btc_1h_staged_quality_summary_v32_v33", quality, "post-btc-1h-staged-quality-summary-v32-v33-20260601"),
        ("open_source_backlearning_pattern_map_v9", pattern_map, "open-source-backlearning-pattern-map-v9-20260601"),
        ("backlearning_multisource_scaffold_v11", backlearning, "backlearning-multisource-scaffold-v11-20260601"),
        ("exploratory_only_backtest_decision_v32_v33", backtest, "exploratory-only-backtest-decision-v32-v33-20260601"),
        ("24h_live_test_preflight_runner_v15", preflight, "24h-live-test-preflight-runner-v15-20260601"),
        ("24h_readiness_master_packet_v32_v33", master, "24h-readiness-master-packet-v32-v33-20260601"),
    ]
    if result is not None:
        specs.insert(1, ("btc_1h_staged_controller_result_v6", result, "btc-1h-staged-controller-result-v6-20260601"))
    outputs: List[str] = []
    for report_type, content, stem in specs:
        outputs.extend(_write_pair(report_type, content, stem, source_paths=source_paths))
    research_index = _index_v32_v33(outputs=outputs, master=master)
    outputs.extend(_write_pair("research_readiness_index_v32_v33", research_index, "research-readiness-index-v32-v33-20260601", source_paths=[*source_paths, *outputs]))
    print(json.dumps({"status": "v32_v33_reports_written", "fetch": args.fetch, "completed_chunks": (result or {}).get("completed_chunk_ids", []), "outputs": outputs}, indent=2, sort_keys=True))
    return 0 if not result or result.get("status") == "btc_1h_staged_controller_result_v6_ready" else 2
