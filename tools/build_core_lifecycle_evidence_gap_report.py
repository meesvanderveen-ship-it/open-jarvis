...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build read-only C4/D1/D2/D3/D4/D5 core lifecycle evidence gap report. No Coinbase calls."
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    report = build_core_lifecycle_evidence_gap_report(root=Path(args.root))
    if args.json_out:
        _atomic_write(Path(args.json_out), (json.dumps(report, indent=2, sort_keys=True, ensure_ascii=True) + "\n").encode("utf-8"))
    if args.markdown_out:
        _atomic_write(Path(args.markdown_out), (render_core_lifecycle_evidence_gap_markdown(report) + "\n").encode("utf-8"))
    if args.markdown:
        print(render_core_lifecycle_evidence_gap_markdown(report))
    else:
